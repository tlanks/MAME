...
static void check_variables()
{
   ...
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
      ...
      if (alternate_renderer)
      {
         ...
         if (pch)
         {
            fb_width = static_cast<int>(strtoul(pch, nullptr, 0));
            fb_pitch = fb_width;
         }
         pch = strtok(nullptr, "x");
         if (pch)
            fb_height = static_cast<int>(strtoul(pch, nullptr, 0));
         
         fprintf(stderr, "[libretro-test]: Got size: %u x %u.\n", fb_width, fb_height);
         retro_aspect = (float)fb_width / (float)fb_height;
         
         if (fb_width / 4 == fb_height / 3)
            res_43 = true;
         else
            res_43 = false;
...
void update_geometry()
{
   ...
   system_av_info.geometry.base_width = static_cast<unsigned int>(fb_width);
   system_av_info.geometry.base_height = static_cast<unsigned int>(fb_height);
...
void retro_get_system_av_info(struct retro_system_av_info *info)
{
   ...
   info->geometry.base_width = static_cast<unsigned int>(fb_width);
   info->geometry.base_height = static_cast<unsigned int>(fb_height);
   
   if (log_cb)
      log_cb(RETRO_LOG_INFO, "AV_INFO: width=%d height=%d\n", info->geometry.base_width, info->geometry.base_height);
   
   info->geometry.max_width = static_cast<unsigned int>(fb_width);
   info->geometry.max_height = static_cast<unsigned int>(fb_height);
   max_width = fb_width;
...
#else
   if (draw_this_frame)
      video_cb(videoBuffer, static_cast<unsigned int>(fb_width), static_cast<unsigned int>(fb_height),
               static_cast<size_t>(fb_pitch << LOG_PIXEL_BYTES));
   else
      video_cb(nullptr, static_cast<unsigned int>(fb_width), static_cast<unsigned int>(fb_height),
               static_cast<size_t>(fb_pitch << LOG_PIXEL_BYTES));
#endif
}
...
//FIXME: re-add way to handle OGL
   ...
   // check ROM validity
   emu_options options;
   options.set_value(OPTION_MEDIAPATH, g_rom_dir, OPTION_PRIORITY_HIGH);
   
   // TODO(sgc): wonder if we can select a working alternative using the auditor?
   
   driver_enumerator drivlist(options, basename);
   ...
   while (drivlist.next())
   {
      ...
      {
         RARCH_LOG("found rom for '%s': %s\n", basename, drivlist.driver().name);
